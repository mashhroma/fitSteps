				...
				<div className="App" onClick={closeForm}>
					...
					<main>
						<Routes>
							...
							<Route
								path="/confidential_policy"
								element={<ConfidentialPolicy />}
							/>
							<Route
								path="/payment_subscription/:subscribeType"
								element={<PaymentPage />}
							/>
					...
					<Footer />
					{(loginFormVisibility ||
						coachRegFormVisibility ||
						regFormVisibility ||
						coachLoginFormVisibility) && <div className="overlay"></div>}
				</div>
				{coachRegFormVisibility && (
					<RegistrationForm role="coach" closeRegForm={toggleCoachRegForm} />
				)}
				{regFormVisibility && (
					<RegistrationForm role="user" closeRegForm={toggleUserRegForm} />
				)}
				{loginFormVisibility && (
					<LoginForm role="user" closeLoginForm={toggleUserLoginForm} />
				)}
				{coachLoginFormVisibility && (
					<LoginForm role="coach" closeLoginForm={toggleCoachLoginForm} />
				)}
			</Router>
		</ContextProvider>
	);
